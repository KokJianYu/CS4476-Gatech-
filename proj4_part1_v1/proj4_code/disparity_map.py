...
def calculate_disparity_map(left_img: torch.Tensor,
                            ...
                            block_size: int, # This should just be an int
  ...
  for i in range(H-block_size+1):
    ...
    for j in range(W-block_size+1):
      similarity_errors = []
      patch1 = left_img[i:i+block_size,j:j+block_size,:]
      for k in range(max_search_bound):
        x_idx = j - k
        if x_idx < 0:
          break
        patch2 = right_img[i:i+block_size,x_idx:x_idx+block_size,:]
        error = sim_measure_function(patch1, patch2)
        similarity_errors = np.append(similarity_errors, error.cpu().detach().numpy())
      if similarity_errors.shape[0] > 0:
        disparity_map[i, j] = torch.tensor(np.argmin(similarity_errors))
...
def calculate_cost_volume(left_img: torch.Tensor,
  ...
  for i in range(H-block_size+1):
    # for every col
    for j in range(W-block_size+1):
      similarity_errors = np.zeros((max_disparity))
      patch1 = left_img[i,j,:]
      for k in range(max_disparity):
        x_idx = j - k
        if x_idx < 0:
          break
        patch2 = right_img[i,x_idx,:]
        error = sim_measure_function(patch1, patch2)
        similarity_errors[k] = error
      cost_volume[i, j, :] = torch.tensor(similarity_errors)

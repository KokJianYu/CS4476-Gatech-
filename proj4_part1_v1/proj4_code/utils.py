...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
  ...
  # Student code begin
  ############################################################################
  center_block = (H//2, W//2)
  im_left = torch.randint(2, (H, W, 1))
  im_left = im_left.repeat(1,1,C)

  im_right = im_left.clone()
  random = torch.randint(2, (block_size[0], disparity, 1)).repeat(1,1,C)
  left_corner = (center_block[0] - block_size[0]//2)
  right_corner = (center_block[0] + block_size[0]//2)
  top_corner = (center_block[1] - block_size[1]//2)
  bottom_corner = (center_block[1] + block_size[1]//2)
  # Move center block of right image by disparity
  im_right[top_corner:bottom_corner, left_corner-disparity:right_corner-disparity] = im_right[top_corner:bottom_corner, left_corner:right_corner]
  im_right[top_corner:bottom_corner,right_corner-disparity:right_corner] = torch.randint(2, (bottom_corner-top_corner, disparity, 1)).repeat(1,1,C)
  return im_left.to(dtype=torch.float32), im_right.to(dtype=torch.float32)

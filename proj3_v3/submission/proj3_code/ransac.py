...
def ransac_fundamental_matrix(x_0s, x_1s):
    ...
    k = 9
    p = 0.7
    num_iters = calculate_num_ransac_iterations(P, k, p)
    num_inliers = 0
    for i in range(num_iters):
        random_points = np.random.choice(x_0s.shape[0], (k))
        result_F = solve_F(x_0s[random_points], x_1s[random_points])
        inliers_idx= find_inliers(x_0s, result_F, x_1s, 1)
        if inliers_idx.shape[0] > num_inliers:
...
def test_with_epipolar_lines():
    ...
    # Rushmore
    image1 = load_image('../data/test_image_0.jpg')
    image2 = load_image('../data/test_image_1.jpg')

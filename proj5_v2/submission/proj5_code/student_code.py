...
def pairwise_distances(X, Y):
    ...
    for i in range(M):
        dists = X - Y[i]
        D[:,i] = np.linalg.norm(dists, axis=1)
...
def get_tiny_images(image_arrays):
    ...
    dim_x = 16
    dim_y = 16
    feats = np.empty((len(image_arrays), dim_x * dim_y))
    for i, image in enumerate(image_arrays):
        resized_image = cv2.resize(image, (dim_x, dim_y))
        feats[i] = resized_image.reshape(dim_x * dim_y)
...
def nearest_neighbor_classify(train_image_feats, train_labels,
    ...
    nearest_3_neighbors = np.argpartition(dists, k, axis=1)[:,:3]
    neighbors_label = np.array(train_labels)[nearest_3_neighbors]
    axis = 1
    u, indices = np.unique(neighbors_label, return_inverse=True)
    test_labels = u[np.argmax(np.apply_along_axis(np.bincount, axis, indices.reshape(neighbors_label.shape),
...
def kmeans(feature_vectors, k, max_iter = 100):
    ...
    init_centroids_id = np.random.choice(unique_feature_vectors.shape[0], size=k, replace=False)
    centroids = unique_feature_vectors[init_centroids_id,:]

    for i in range(max_iter):
        labels = kmeans_quantize(feature_vectors, centroids)
        for j in range(k):
            labels_idx = (labels == j)
            if np.any(labels_idx):
                centroids[j,:] = feature_vectors[labels_idx,:].mean(axis=0)
...
def build_vocabulary(image_arrays, vocab_size, stride = 20):
    ...
    for i,image in enumerate(image_arrays):
        ...
        width = image.shape[1]
        x = np.arange(10, width-10+1, stride)
        y = np.arange(10, height-10+1, stride)
        ...
        img_tensor = torch.unsqueeze(img_tensor, dim=0)
        img_tensor = torch.unsqueeze(img_tensor, dim=0)
        features = get_siftnet_features(img_tensor, x_list, y_list)
        #print(features.shape)
        fvs += list(features)
    fvs = np.array(fvs)
    vocab = kmeans(fvs, vocab_size)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return vocab
...
def get_bags_of_sifts(image_arrays, vocabulary, step_size = 10):
    ...
    for i,image in enumerate(image_arrays):
        height = image.shape[0]
        width = image.shape[1]
        x = np.arange(10, width-10+1, step_size)
        y = np.arange(10, height-10+1, step_size)
        x_list, y_list = np.meshgrid(x,y)
        x_list = x_list.flatten()
        y_list = y_list.flatten()
        img_tensor = torch.tensor(image).float()
        ...
        #print(features.shape)
        fvs += list(features)
        num_of_points += [features.shape[0]]
    fvs = np.array(fvs)
    
    labels = kmeans_quantize(fvs, vocab)
    current_index = 0
    for i in range(num_images):
        label_for_image = labels[current_index:current_index+num_of_points[i]] 
        histogram = np.histogram(label_for_image, bins=np.arange(vocab_size+1))[0]
        feats[i,:] = histogram / np.linalg.norm(histogram)
        current_index += num_of_points[i]
